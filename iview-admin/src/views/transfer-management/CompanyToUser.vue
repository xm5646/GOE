    ...
    export default {
        ...
        methods: {
            ...
            getAllListByPage (page) {
                this.doGet({url: this.APIServer + '/goeIndexConsumeController/findAllTransferByConsumeType?code=1&pageNum=' + page}).then(result => {
                    if (result.success) {
            ...
            getListByAccountAndPage (page) {
                this.doGet({
                    url: this.APIServer + '/goeIndexConsumeController/findByAccountAndConsumeType?code=1&account=' + this.searchAccount + '&pageNum=' + page
                }).then(result => {
                    if (result.success) {
                        this.pageObj.totalPage = result.data.totalPages;
                        this.pageObj.totalCount = result.data.totalElements;
