    ...
    export default {
        ...
        methods: {
            ...
            getAllListByPage (page) {
                this.doGet({url: this.APIServer + '/goeIndexOrderController//findAllOrdersOfProductCoin?pageNum=' + page}).then(result => {
                    if (result.success) {
            ...
            getListByAccountAndPage (page) {
                this.doGet({
                    url: this.APIServer + '/goeIndexOrderController/findAllOrdersOfProductCoinByAccount?account=' + this.searchAccount + '&pageNum=' + page
                }).then(result => {
                    if (result.success) {
                        let tmpList = result.data.content;
                        for (let i = 0; i < result.data.content.length; i++) {
