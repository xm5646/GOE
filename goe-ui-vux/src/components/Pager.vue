  ...
  <div class="weui-cell">
    ...
    <div class="weui-cell__ft vux-cell-primary" :class="{'vux-number-round': buttonStyle === 'round'}" v-show="!readonly" style="font-size:0">
      <div :style="{float:align}">
        <x-button mini style="float: left" @click.native="goFirst" :disabled="disabledMin">首页</x-button>
        ...
        <input v-model.number="currentValue" :name="name" class="vux-number-input" :style="{width: width}" :readonly="!fillable" pattern="[0-9]*" type="number" @blur="blur" @focus="changeNumber"/>
        ...
        <x-button mini @click.native="goLast" :disabled="disabledMax">末页</x-button>
      </div>
    </div>
    &nbsp;&nbsp;<span>共 {{max}} 页</span>
