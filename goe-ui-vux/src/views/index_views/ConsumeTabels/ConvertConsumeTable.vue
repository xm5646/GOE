...
//      this.getPage(1)
    ...
    methods: {
      ...
      getPage (pageNum) {
        console.log(pageNum)
        const url = GoeConfig.apiServer + '/consumerRecord/findByAccountAndConsumeType?account=' + JSON.parse(window.localStorage.getItem('User')).account + '&consumeTypeCode=2' + '&pageNum=' + (pageNum - 1)
        this.$http.get(url,
          {
            _timeout: 10000,
            onTimeout: (request) => {
